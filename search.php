...
if($mybb->input['action'] == "results")
{
	...
	if($search['resulttype'] == "threads")
	{
		...
		if($search['querycache'] != "")
		{
			...
			$query = $db->simple_select("threads t", "t.tid", $where_conditions." AND {$unapproved_where_t} AND t.moved='0' ORDER BY t.lastpost DESC {$limitsql}");
		...
		{
			$where_conditions = "t.tid IN (".$search['threads'].")";
			$query = $db->simple_select("threads t", "COUNT(t.tid) AS resultcount", $where_conditions." AND {$unapproved_where_t} AND t.moved='0' {$limitsql}");
		...
		output_page(\MyBB\template('search_results_threads', [
			'multipage' => $multipage,
			'results' => $results,
			'threads' => $threads,
		]));
	...
	else // Displaying results as posts
	{
		...
		if(!empty($pids))
		{
			...
			$query = $db->simple_select("threads", "tid", "tid IN(".$db->escape_string(implode(',', array_keys($tids))).") AND (NOT ({$unapproved_where}){$permsql} OR moved != '0')");
			while($thread = $db->fetch_array($query))
			{
		...
		output_page(\MyBB\template('search_results_posts', [
			'multipage' => $multipage,
			'results' => $results,
			'posts' => $posts,
		]));
...
else
{
	...
	output_page(\MyBB\template('search_search', [
		'search' => $search,
		'forums' => $forums,
		'prefixes' => $prefixes,
	]));
