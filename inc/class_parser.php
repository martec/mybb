...
class postParser
{
	...
	function cache_smilies()
	{
		...
		if(is_array($smilies))
		{
			foreach($smilies as $sid => $smilie)
			{
				...
				{
					$s = $this->parse_html($s);
					$this->smilies_cache[$s] = template('parser_mycodes_smilie', [
						'smilie' => $smilie
					]);
	...
	function mycode_handle_size($size, $text)
	{
		...
		return template('parser_mycodes_size', [
			'text' => $text,
			'size' => $size
		]);
	}
	...
	function mycode_parse_post_quotes($message, $username, $text_only = false)
	{
		...
		if(isset($match[1]) && (int)$match[1])
		{
			if($match[1] < TIME_NOW)
			{
				if($text_only)
				{
					$quote['date'] = my_date('normal', (int)$match[1]);
				}
				else
				{
					$quote['date'] = my_date('relative', (int)$match[1]);
				}
			}
			$quote['username'] = preg_replace("#(?:&quot;|\"|')? dateline=(?:&quot;|\"|')?[0-9]+(?:&quot;|\"|')?#i", '', $quote['username']);
			$delete_quote = false;
		}

		if($delete_quote)
		{
			$quote['username'] = my_substr($quote['username'], 0, my_strlen($quote['username']) - 1);
		}

		if(!empty($this->options['allow_html']))
		{
			$quote['username'] = htmlspecialchars_uni($quote['username']);
		}
		...
		{
			return template('parser_mycodes_quote', [
				'quote' => $quote,
	...
	function mycode_parse_code($code, $text_only = false)
	{
		...
		return template('parser_mycodes_code', [
			'code' => $code
		]);;
	}
	...
	function mycode_parse_php($str, $bare_return = false, $text_only = false)
	{
		...
		return template('parser_mycodes_php', [
			'code' => $code
		]);
	}
	...
	*/
	...
	function mycode_parse_img($url, $dimensions = array(), $align = '')
	{
		...
		$image['url'] = str_replace("\r", "", $image['url']);

		if(!empty($this->options['allow_html']))
		{
			$image['url'] = $this->parse_html($url);
		}
		...
		$image['alt'] = basename($url);
		$image['alt'] = htmlspecialchars_decode($image['alt']);
		...
		return template('parser_mycodes_img', [
			'image' => $image
		]);
	}
	...
	*/
	function mycode_parse_email($email, $name="")
	{
		...
		return template('parser_mycodes_email', [
			'email' => $email,
			'name' => $name
		]);
	}
	...
	function mycode_parse_video($video, $url)
	{
		...
		return template('parser_mycodes_video', [
