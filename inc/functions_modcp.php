...
function modcp_can_manage_user($uid)
{
	...
	elseif($user_permissions['cancp'] == 1 && ($mybb->usergroup['cancp'] != 1 || (is_super_admin($uid) && !is_super_admin($mybb->user['uid']))))
	{
		return false;
	}
	return true;
...
{
	global $mybb, $db, $lang, $theme, $announcements, $announcements_forum, $moderated_forums, $unviewableforums, $parser;
			...
			else
			{
				...
				if(isset($announcements[$forum['fid']]))
				{
					foreach($announcements[$forum['fid']] as $aid => $announcement)
					{
						...
						$currentAnnouncements[$aid] = $announcement;
					}
				}
			}
